...
class TestYExprToken(utils.BaseTestCase):
    def test_operator_operands(self):
        ...
        for i, operands in enumerate(Token.operator_operands(input_v)):
...
class TestYExprFnFile(utils.BaseTestCase):
    ...
    @mock.patch('hotsos.core.ycheck.engine.properties.requires'
                ...
                return_value=42)
    def test_prop_exists(self, mock_property):
        uut = YExprFnFile(
    ...
    def test_prop_does_not_exists(self):
        uut = YExprFnFile(
...
#  ___________________________________________________________________________#
    ...
    def test_service_does_not_exists(self):
        ...
        self.assertEqual(str(uut.eval()), str(YNotFound('does_not_matter')))
...
class TestYExprFnReadIni(utils.BaseTestCase):
    ...
    def test_ini_does_not_exists(self, _):
        ...
        self.assertTrue(isinstance(uut.eval(), YNotFound))
...
class TestYExprBoolean(utils.BaseTestCase):
    ...
    def test_yexpr_boolean_true(self):
        uut = YExprBoolean(["TrUe"])
        self.assertTrue(uut.eval())

    def test_yexpr_boolean_false(self):
        uut = YExprBoolean(["fAlSe"])
        self.assertFalse(uut.eval())

    def test_yexpr_boolean_garbage(self):
        uut = YExprBoolean(["not-a-boolean"])
        with self.assertRaises(Exception):
            uut.eval()
...
class TestYExprStringLiteral(utils.BaseTestCase):
    def test_yexpr_string_literal(self):
        uut = YExprStringLiteral(["does-not-matter"])
...
class TestYExprInteger(utils.BaseTestCase):
    ...
    def test_yexpr_integer_not_an_int(self):
        uut = YExprInteger(["garbage"])
        with self.assertRaises(Exception):
            uut.eval()

    def test_yexpr_integer_not_an_int_float(self):
        uut = YExprInteger(["1.1"])
        with self.assertRaises(Exception):
            uut.eval()
...
class TestYExprRuntimeVariable(utils.BaseTestCase):
    @mock.patch('hotsos.core.ycheck.engine.properties.requires'
                '.types.expr.YExprRuntimeVariable.get_property',
                return_value=1.1)
    def test_yexpr_runtime_variable_exists(self, _):
        ...
        uut = YExprRuntimeVariable(["@thisx.is.a.test"])
...
class TestYExprValue(utils.BaseTestCase):
    ...
    def test_yexpr_value_v(self):
        class MockValueType(YExprValueBase):
            def eval(self):
                return 42

        uut = YExprValue([MockValueType([None])])
        self.assertEqual(uut.eval(), 42)
    ...
    def test_yexpr_value_f(self):
        ...
        uut = YExprValue([MockFnType([None])])
        self.assertEqual(uut.eval(), 42)
...
class TestYExprParser(ppt.TestParseResultsAsserts, utils.BaseTestCase):
    ...
    @mock.patch('hotsos.core.ycheck.engine.properties.requires'
                '.types.expr.FileObj.mtime', new_callable=PropertyMock,
                return_value=42)
    def test_fn_file_prop_exists(self, _):
        self.expect_true("file('/etc/dummy', 'mtime') == 42")

    def test_fn_file_prop_does_not_exist(self):
        # Implicit false, because attribute  does not exist
        self.expect_false("file('/etc/dummy', 'mtime') == 42")
    ...
    def test_fn_systemd_service_does_not_exist(self):
        self.expect_isinstance("systemd('dummy-service')", YNotFound)
    ...
    @mock.patch('hotsos.core.ycheck.engine.properties.requires'
                '.types.expr.IniConfigBase.get',
                return_value=42)
    def test_fn_read_ini_read_by_key(self, _, __):
        self.assertEqual(
    ...
    @mock.patch('hotsos.core.ycheck.engine.properties.requires'
                '.types.expr.IniConfigBase.get',
                return_value=56)
    def test_fn_read_ini_read_by_key_section(self, _, __):
        self.assertEqual(
    ...
    @mock.patch('hotsos.core.ycheck.engine.properties.requires'
                '.types.expr.IniConfigBase.get',
                return_value=None)
    def test_fn_read_ini_read_by_key_default(self, _, __):
        self.assertEqual(
    ...
    def test_fn_read_cert_does_not_exist(self):
        self.expect_isinstance("read_cert('/etc/dummy-cert')", YNotFound)
